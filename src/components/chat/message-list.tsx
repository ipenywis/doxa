...
import type { ChatMessage, MessagePart, ToolPart } from "@/src/components/chat";
...
const markdownComponents: Components = {
  ...
  pre: ({ children }: { children?: React.ReactNode }) => (
    <pre className="chat-code-block overflow-x-auto rounded-md border border-border/30 bg-muted/80 p-3">
      {children}
    </pre>
  ),
...
export function MessageList({
  ...
  const showLoadingDots = isLoading && !isStreaming && lastMsg?.role === "user";
          ...
          if (msg.role === "assistant") {
            ...
            return (
              <div key={msg.id} className="flex flex-col gap-1">
                ...
                {agentStatus && <AgentActivityFooter status={agentStatus} />}
              </div>
            );
          }
